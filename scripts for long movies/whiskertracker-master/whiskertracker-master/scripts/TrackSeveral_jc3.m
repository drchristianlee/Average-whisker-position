function vargout=TrackSeveral_jc3(g_tWT)
...
switch condchoice
    case 'Yes entire List'
        ...
        slash = findstr(g_tWT.MovieInfo.Filename, '\');
        cd (g_tWT.MovieInfo.Filename(1:slash(end)));
        load ('movie1list.mat');
        track_average_whisker_position_jc(g_tWT, g_tWT.MovieInfo)
        for n=2:length(files)
            wt_load_movie(files{n})
            track_average_whisker_position_jc(g_tWT, g_tWT.MovieInfo)
        end
